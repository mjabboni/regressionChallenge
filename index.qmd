...
- **Intercept (β₀):** Approximately 0.3810
- **Coefficient on Time (β₂):** Approximately 5.3651
- **R-squared:** Very high (close to 1.0)

**Comparison to True Relationship:**
The true coefficient on Time is 0.1, but our estimate is 5.3651—over 50 times larger! This massive overestimate occurs because Time is highly correlated with Stress in this dataset. When we regress Anxiety on Time alone, the coefficient captures both the direct effect of Time (0.1) and the indirect effect through its correlation with Stress.

This is a classic example of omitted variable bias: by failing to control for Stress, we get a completely misleading estimate of Time's effect on Anxiety.
...
The scatter plot shows a strong positive relationship between Time and Anxiety, with an R-squared near 1.0. The regression line fits the data well.

**Critical Issues:**

1. **Severe omitted variable bias:** The coefficient of 5.3651 is astronomically wrong compared to the true value of 0.1. This happens because Time is correlated with Stress—people with higher stress levels tend to spend more time on social media, and Stress has a much larger effect on Anxiety (coefficient = 1.0) than Time does (coefficient = 0.1).

2. **Misleading interpretation:** Someone looking at this bivariate regression would conclude that Time has a massive effect on Anxiety (5.3651 units per minute), when the true effect is tiny (0.1 units per minute). This is exactly why we need to control for Stress in multiple regression.

3. **Perfect fit is deceptive:** The high R-squared suggests a good model, but the coefficient is completely wrong. This demonstrates that statistical significance and R-squared don't guarantee correct coefficient estimates.
...
- **Intercept (β₀):** Approximately 0.7333
- **Coefficient on StressSurvey (β₁):** Approximately 0.9556
- **Coefficient on Time (β₂):** Approximately -1.8222
- **R-squared:** Very high (close to 1.0)
...
|------------|-----------|----------------|------------|
| Intercept | 0.0 | 0.7333 | Wrong |
| Stress | 1.0 | 0.9556 | Close, but using wrong variable |
| Time | 0.1 | **-1.8222** | **WRONG SIGN!** |

**Critical Finding:**
1. **Wrong sign for Time:** The true coefficient is +0.1 (Time increases Anxiety), but the regression estimates -1.8222 (Time decreases Anxiety). This is a complete reversal of the true relationship!
...
Both models have R-squared values near or at 1.0, indicating excellent statistical fit. However, Model 2 (Stress + Time) has a perfect R-squared of 1.0, while Model 1 (StressSurvey + Time) is slightly lower but still extremely high.

**Coefficient Comparison:**

| Coefficient | Model 1 (StressSurvey) | Model 2 (Stress) | True Value | Correct? |
|------------|------------------------|------------------|------------|----------|
| Intercept | 0.7333 | 0.0000 | 0.0 | Model 2 only |
| Stress Coef | 0.9556 | 1.0000 | 1.0 | Model 2 only |
| Time Coef | **-1.8222** | 0.1000 | 0.1 | **Model 2 only** |
...
4. **Policy implications:** If Model 1's results were published, policymakers might conclude that reducing social media time increases anxiety (because the Time coefficient is negative). This is the opposite of the truth! Real-world decisions based on such results could be harmful.
...
> 
> *"A groundbreaking study published in a leading psychology journal has found that increased time on social media platforms is associated with lower anxiety levels. The research, which controlled for stress levels through comprehensive survey data, revealed that each additional minute spent on social media reduces anxiety by nearly 2 points. 'This challenges everything we thought we knew about social media and mental health,' said the lead researcher."*
...
1. **Typical Parents:** Parents will likely believe **Model 1** (the wrong one) because of confirmation bias working in an unexpected direction:
   ...
   - Model 1's larger coefficient (-1.8222) creates a more dramatic narrative than Model 2's small coefficient (0.1)
...
The Time coefficient (0.1000) is now **correct**! This is a huge improvement over the full-sample result (-1.8222). However, the StressSurvey coefficient (0.3333) doesn't match the true Stress coefficient (1.0) because StressSurvey is still a proxy with a different scale.
...
ax.set_title('Full Sample: Time Coef = -1.8222 (WRONG!)', fontsize=12, fontweight='bold', color='red')
ax.grid(True, alpha=0.3)
ax.text(0.5, 0.95, 'Estimated β₂ = -1.8222\n(True value: +0.1)', 
...
3. **The sign matters:** Getting the wrong sign on a coefficient (like Time going from +0.1 to -1.8222) can lead to completely opposite policy recommendations and real-world harm.
