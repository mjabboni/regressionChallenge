...
print("=" * 60)
print(f"\nIntercept (beta_0): {model1_sklearn.intercept_:.4f}")
print(f"Coefficient on StressSurvey (beta_1): {model1_sklearn.coef_[0]:.4f}")
...
ax.text(0.05, 0.95, f'R^2 = {r2:.4f}\nbeta_1 = {model1_sklearn.coef_[0]:.4f}', 
...
ax.text(0.05, 0.95, f'R^2 = {r2:.4f}\nbeta_2 = {model3_sklearn.coef_[0]:.4f}', 
        transform=ax.transAxes, fontsize=11,
...
print(f"Coefficient on StressSurvey (beta_1): {model5_sklearn.coef_[0]:.4f}")
print(f"Coefficient on Time (beta_2): {model5_sklearn.coef_[1]:.4f}")
print(f"R-squared: {r2_score(y, model5_sklearn.predict(X_mult1)):.4f}")
print("\n" + "=" * 60)
print("TRUE COEFFICIENTS FOR COMPARISON:")
print("=" * 60)
print("True Intercept (beta_0): 0.0000")
print("True Stress coefficient (beta_1): 1.0000")
print("True Time coefficient (beta_2): 0.1000")
print("\n" + "=" * 60)
print("STATSMODELS DETAILED OUTPUT:")
print("=" * 60)
...
print("=" * 60)
print(f"\nIntercept (beta_0): {model6_sklearn.intercept_:.4f}")
print(f"Coefficient on Stress (beta_1): {model6_sklearn.coef_[0]:.4f}")
print(f"Coefficient on Time (beta_2): {model6_sklearn.coef_[1]:.4f}")
...
print("=" * 80)
print("Intercept (beta_0): 0.0")
print("Stress coefficient (beta_1): 1.0")
print("Time coefficient (beta_2): 0.1")
print("\n" + "=" * 80)
...
print("=" * 60)
print(f"Intercept (beta_0): {model_subset_sklearn.intercept_:.4f}")
print(f"Coefficient on StressSurvey (beta_1): {model_subset_sklearn.coef_[0]:.4f}")
print(f"Coefficient on Time (beta_2): {model_subset_sklearn.coef_[1]:.4f}")
...
print("=" * 60)
print(f"Intercept (beta_0): {model_subset2_sklearn.intercept_:.4f}")
print(f"Coefficient on Stress (beta_1): {model_subset2_sklearn.coef_[0]:.4f}")
print(f"Coefficient on Time (beta_2): {model_subset2_sklearn.coef_[1]:.4f}")
...
The following plots show scatter plots of Time vs Anxiety for the full sample and the subset. While these are bivariate views, the coefficient annotations reflect the results from the multiple regression models (controlling for StressSurvey in the full sample, and controlling for Stress in the subset). The plots illustrate how the same data, when analyzed with different approaches, produces dramatically different coefficient estimates.

```{python}
#| label: fig-full-sample-wrong
#| fig-cap: "Full sample data showing incorrect Time coefficient from multiple regression (Anxiety ~ StressSurvey + Time)"
#| echo: false
#| warning: false
...
ax.grid(True, alpha=0.3)
ax.text(0.5, 0.95, 'Estimated beta_2 = -2.7799\n(True value: +0.1)', 
...
The first plot shows all 15 observations from the full sample. The scatter plot displays the bivariate relationship between Time and Anxiety, while the annotation shows the Time coefficient from the multiple regression model (Anxiety ~ StressSurvey + Time), which incorrectly estimates -2.7799 instead of the true value of +0.1.

```{python}
#| label: fig-subset-correct
#| fig-cap: "Subset data (Stress ≤ 2) showing correct Time coefficient from multiple regression (Anxiety ~ Stress + Time)"
#| echo: false
#| warning: false
...
ax.grid(True, alpha=0.3)
ax.text(0.5, 0.95, 'Estimated beta_2 = 0.1000\n(True value: +0.1)', 
         transform=ax.transAxes, fontsize=10,
         verticalalignment='top', ha='center',
...
The second plot shows the 9 observations from the subset (Stress ≤ 2). This scatter plot displays the same bivariate view, but the annotation shows the Time coefficient from the multiple regression model using the true Stress variable (Anxiety ~ Stress + Time), which correctly estimates 0.1000, matching the true value.
